...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "primary persona"
      decision: "the coffee drinker; café side minimal in MVP"
    - topic: "pain type"
      decision: "workflow friction + progress trapped on paper"
    - topic: "insight"
      decision: "aggregation (wallet) + notifications + zero-friction for tiny cafés"
    - topic: "customer auth"
      decision: "account login (method deferred to stack selection)"
    - topic: "café trust model"
      decision: "separate authenticated café/barista role; customer shows QR, café scans"
    - topic: "MVP scope"
      decision: "committed to longer timeline (~8 wks); fuller two-sided MVP with push"
    - topic: "secondary success"
      decision: "notification drives a measurable return visit"
    - topic: "café onboarding"
      decision: "operator-onboarded only in MVP; no public café self-registration"
    - topic: "one-stamp-away nudge"
      decision: "must-have in MVP (part of retention thesis)"
    - topic: "added FRs"
      decision: "stamp reversal/correction; account recovery preserving stamps"
    - topic: "café auth mechanism (Socrates FR-002)"
      decision: "per-café device token, not per-barista accounts"
    - topic: "stamp credential (Socrates FR-006/FR-007)"
      decision: "QR or one-time/rotating customer code; both customer-presented"
    - topic: "stamp reversal (Socrates FR-008)"
      decision: "kept but audit-logged"
    - topic: "redemption flow (Socrates FR-010)"
      decision: "self-serve, one-time system-enforced; residual fraud Q tracked"
    - topic: "café onboarding (Socrates FR-011)"
      decision: "self-register + manual operator approval (supersedes operator-only)"
    - topic: "account recovery (Socrates FR-003)"
      decision: "deferred to nice-to-have; tension with durability guardrail tracked"
    - topic: "post-redeem behavior"
      decision: "card resets but archives to per-café history; café tax export (FR-015/016)"
    - topic: "stamp expiry"
      decision: "per-café program policy (configurable)"
    - topic: "stamp rate"
      decision: "one stamp per qualifying item (café enters quantity)"
    - topic: "data retention"
      decision: "retain completed-card history indefinitely"
    - topic: "product framing"
      decision: "mobile app; medium scale; no deadline; after-hours side project"
    - topic: "non-goals"
      decision: "no per-barista accounts; no payment/POS; recovery deferred; no café self-go-live"
  frs_drafted: 16
...
# Shape Notes — Appretiate

> Seed idea (verbatim): Appretiate is a mobile app that replaces paper loyalty stamp
> cards from cafés and coffee shops. Users collect all their loyalty cards in one place
> on their phone. Problem: customers miss free-coffee rewards because they forget, lose,
> or skip paper cards; cafés lose a retention tool. How it works: a user opens a digital
> stamp card for any participating café; when they buy a coffee, the barista scans a QR
> code from the app to add a stamp; once enough stamps are collected (e.g. 9), the user
> is notified and can redeem a free coffee on the next visit. User benefits: no forgotten
> cards, all programs in one app, progress at a glance, reward notifications. Café benefits:
> repeat customers, no printing costs, a direct notification channel to regulars.
...
counter. Progress lives on a single physical card that can't be backed up, synced, or
seen at a glance, so a half-finished card is invisible until the customer happens to be
holding it at the right café. Customers forfeit free coffees they were owed; cafés lose
the repeat-visit pull the card was meant to create.

The insight is that the value is not digitizing any one café's card — it is the wallet
plus the nudge. A single app holding every participating café's card beats any one café's
...
> Scale note (Phase 6 Socrates probe): at ~100x growth the core stamp→reward rule is
> expected to hold unchanged; only operational load grows (café-approval throughput,
> fraud pressure on self-serve redemption / one-time codes, notification-frequency tuning).
...
is at the counter, paying for a coffee: they open the app so the barista can add a stamp,
and they glance at how close they are to a reward.

The café (owner/barista) is a **secondary** actor in the MVP — present only insofar as
they need to add a stamp to a customer's card. Their experience is deliberately minimal in
the first version (see Non-Goals / Access Control).

## Access Control

Multi-user with three roles and a clear trust boundary around the value-granting action.
(Role model refined during the Phase 4.5 Socrates round.)

- **Customer** — signs up / signs in with an account (email + password, OAuth, or
  passwordless; exact method deferred to stack selection). Can open/hold café stamp cards,
  present a QR or one-time code to receive a stamp, view progress across all their cards,
  and redeem a reward self-serve once eligible (system enforces one-time redemption).
  Cannot grant stamps to themselves.
- **Café** — authenticates into a café-scoped role via a per-café device credential/token
  (not per-barista accounts). Can scan a customer's QR or enter their one-time code to add
  a stamp to that customer's card for this café, and can reverse/correct a mistaken stamp
  (audit-logged). Cannot view or alter unrelated customers' data beyond the stamping
  interaction.
...
role. Customers present a QR or one-time code; the café side scans or enters it. This
direction (customer presents, café verifies) is what prevents self-granted stamps.
Redemption is self-serve but one-time and system-enforced (see FR-010 and Open Questions #2).

Smallest viable model: these three roles. No per-barista accounts, no multi-barista
accountability, no café-staff management in the MVP (deferred — see Non-Goals).

## Success Criteria

### Primary
- The core loop works end to end for a real customer at a real café: open a card →
  present QR → café scans → stamp lands and persists → progress visible → reach the
  reward threshold → redeem a free coffee (self-serve, one-time, system-enforced). If this
  loop holds reliably, the product worked.

### Secondary
- A "reward ready" / "one stamp away" notification measurably brings a customer back for
  a return visit — evidence the notification is functioning as the retention engine.
  (Nice to have; not sufficient on its own.)

### Guardrails
- **No self-granted / fake stamps** — a stamp can only originate from an authenticated
  café action; customers cannot fabricate stamps. Failure here is a regression even if
  the loop otherwise works.
...
Acknowledged on 2026-06-24: ~6–10-week MVP (planning anchor: 8 weeks) requires sustained
after-hours dedication; user explicitly accepted the sustained-effort cost in lieu of
scoping down. The fuller two-sided MVP (push notifications from the start) was chosen with
eyes open. Note: café onboarding was subsequently scoped to operator-gated (no public
café self-registration in MVP) during Phase 4 — see Non-Goals.
...
- FR-002: Café can authenticate into a café-scoped role via a per-café device credential/token (not per-barista accounts). Priority: must-have
...
### History & records
- FR-015: Customer can view their per-café history of completed cards (lifetime coffees / rewards earned at each café). Priority: must-have
- FR-016: Café can generate and print a PDF report of completed cards as line items (date and reward value per card) over a selected period, for tax/accounting documentation. Priority: must-have
  > Resolved (Phase 7): format is a printable PDF; granularity is per completed-card line
  > items (date + reward value); the report is period-selectable. Still open: which
  > jurisdiction's accounting/tax format the report must satisfy — see Open Questions #3.

### Notifications
- FR-013: Customer is notified (push) when a reward is ready. Priority: must-have
...
- FR-014: Customer gets a "one stamp away" nudge. Priority: must-have
...
printable/exportable record of completed cards for tax and accounting documentation. A
fresh card then begins for the next cycle.

This is a per-card workflow with a threshold calculation and a one-time-redemption gate —
not empty CRUD. The empty-CRUD anti-pattern does not apply.

## Non-Goals

- **No per-barista accounts / staff management** — the café authenticates as a device/role
  (FR-002), not individual employees. Per-staff accountability is out of MVP scope.
- **No payment / POS integration** — the app never processes payments or integrates with
  the till; the café only records stamps. This keeps Appretiate off the critical
  transaction path and out of payment-compliance scope.
...
(Not selected as non-goals, so deliberately left open for now: café discovery surface
beyond opening a card, and any cross-café stored value / gift-card concept.)
...
2. **How is self-serve redemption (FR-010) prevented from being triggered without an actual
   ...
   binding, or a lightweight café tap to acknowledge.
3. **Which jurisdiction's accounting/tax format must the café PDF export (FR-016) satisfy?**
   — Partially resolved (Phase 7): format = printable PDF, granularity = per completed-card
   line items (date + reward value), period-selectable. Still open: the specific
   jurisdiction's required fields / certification and any retention rules. Owner: user.
   Deferrable until a target market is chosen.

## Quality cross-check

Run on 2026-06-25. Result: **accepted** — no quality-gate gaps.

- Access Control: present (customer / café-device / operator roles; value-granting action gated).
- Business Logic: present (single declarative domain rule; not empty CRUD).
- Project artifacts: present (this file, valid checkpoint frontmatter).
- Timeline-cost acknowledged: present (8-week MVP; sustained-effort cost explicitly accepted — see Timeline acknowledgment).
- Non-Goals: present (4 entries).
- Preserved behavior: n/a (greenfield).

Tracked open questions carried to /10x-prd (not gate failures): OQ#1 (recovery vs.
durability guardrail), OQ#2 (self-serve redemption fraud), OQ#3 (tax-export jurisdiction —
format/granularity already resolved).
...
- **When** they present their QR code (or one-time code) and the café scans or enters it
- **Then** a stamp is added to that card and the updated progress is immediately visible
  to the customer
...
- A replayed or duplicate scan of the same presented credential within a short window does
  not double-count.
- The stamp that reaches the threshold triggers the reward-ready state (FR-009/FR-013)
  rather than an extra stamp beyond the threshold.
...
- **Then** the reward is marked redeemed exactly once, a glanceable confirmation is shown
  to the café, and the card advances per the program's rules (reset behavior defined in
  Business Logic)

#### Acceptance Criteria
- A completed reward can be redeemed exactly once; a second attempt is rejected by the
  system (the no-double-redemption guardrail holds regardless of self-serve flow).
